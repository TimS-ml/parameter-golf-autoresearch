...
class Hyperparameters:
    ...
    def from_env(cls, env: Mapping[str, str] | None = None) -> "Hyperparameters":
        ...
        return cls(
            ...
            qk_gain_init=float(env_map.get("QK_GAIN_INIT", str(defaults.qk_gain_init))),
            ...
            num_kv_heads=int(env_map.get("NUM_KV_HEADS", str(defaults.num_kv_heads))),
            model_dim=int(env_map.get("MODEL_DIM", str(defaults.model_dim))),
            num_heads=int(env_map.get("NUM_HEADS", str(defaults.num_heads))),
            mlp_mult=int(env_map.get("MLP_MULT", str(defaults.mlp_mult))),
            tie_embeddings=bool(int(env_map.get("TIE_EMBEDDINGS", "1"))),
            rope_base=float(env_map.get("ROPE_BASE", str(defaults.rope_base))),
            logit_softcap=float(env_map.get("LOGIT_SOFTCAP", str(defaults.logit_softcap))),
            tied_embed_init_std=float(env_map.get("TIED_EMBED_INIT_STD", str(defaults.tied_embed_init_std))),
...
def ensure_single_process_env(env: Mapping[str, str] | None = None) -> None:
    ...
    if any(key in env_map for key in distributed_keys):
        ...
        raise RuntimeError(f"train_gpt_mac.py is single-process only; found distributed env: {present}")
...
def maybe_export_artifacts(args: Hyperparameters, base_model: torch.nn.Module, code: str, eval_val_fn, log0) -> None:
    ...
    torch.save(base_model.state_dict(), "final_model_mac.pt")
    model_bytes = os.path.getsize("final_model_mac.pt")
    ...
    with open("final_model_mac.int8.ptz", "wb") as f:
        f.write(quant_blob)
    quant_file_bytes = os.path.getsize("final_model_mac.int8.ptz")
    ...
    with open("final_model_mac.int8.ptz", "rb") as f:
